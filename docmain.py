...
def get_query_plan(user_query, schema):
    ...
    3. SORTING:
    ONLY SORT WHEN SPECIFICALLY MENTIONED!
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt == max_retries - 1:
                st.error(f"⏱️ LLM timeout after {max_retries} attempts. Please try a simpler query.")
                return None
        except Exception as e:
            st.error(f"⚠️ Could not parse query: {e}")
            return None
...
def sanitize_plan(plan, schema=None):
    ...
    if "filter" in plan:
        # FIX: Add filter_logic if it's a list but missing logic
        if isinstance(plan["filter"], list) and "filter_logic" not in plan:
            plan["filter_logic"] = "AND"  # Default to AND
...
def apply_single_filter(df, filter_dict):
    ...
    if pd.api.types.is_datetime64_any_dtype(df[col]):
        ...
        if op == "month":
            ...
            return valid_df[valid_df[col].dt.month == int(val)]
        elif op == "year":
            return valid_df[valid_df[col].dt.year == int(val)]
        elif op == "between":
            start_date = pd.to_datetime(val[0])
            end_date = pd.to_datetime(val[1])
...
if uploaded_file is not None:
    ...
    with tab1:
        ...
        with col1:
            ...
            st.markdown("- *Show top 5 employees by salary*")
            st.markdown("- *Average salary by department*")
            st.markdown("- *Male employees who are data scientists*")
            st.markdown("- *Employees born in March*")
        with col2:
            st.markdown("**Supported operations:**")
            st.markdown("- Filtering (equals, contains, gt, lt)")
            st.markdown("- Aggregations (mean, sum, count)")
            st.markdown("- Sorting & limiting")
            st.markdown("- Date operations (month, year)")
        
        user_query = st.text_input("🔎 Enter your query:", key="nlp_query", placeholder="e.g., Show top 10 products by revenue")
    ...
    with st.expander("💡 See Example Usage"):
        ...
        **Complex Queries:**
        ...
        - "Employees who joined between 2020 and 2023"
        ...
        **Example insights:**
        - Salary vs Years of Experience correlation
    ...
    with st.expander("🔧 Technical Features"):
        ...
        1. **Smart Date Detection**
           - Automatically detects 17+ date formats
           - Uses LLM + regex validation
        ...
        5. **Visualization Intelligence**
           ...
           - Skips high-cardinality categoricals
           - LLM-powered trend explanation
        """)

st.markdown("---")
st.markdown("*Powered by Ollama LLM (qwen2.5:1.5b) | Built with Streamlit | Smart Date Parsing | Fuzzy Matching*")